...
module.exports.getRandomWord = async (rating_threshold, length) => {
    ...
    let list, winner
    while (!winner) {
        list = (await axios.get(endpoint)).data.list
        .sort((a, b) => {
    ...
    solution.hint = solution.example.replace(new RegExp(solution.word, "g"), '~~ Todays Word ~~').replace(new RegExp(solution.word+'s', "g"), '~~ Todays Word ~~')
